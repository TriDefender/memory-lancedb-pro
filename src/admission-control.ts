...
export interface AdmissionControlConfig {
  ...
  /**
   * Per-call chunk bound for the batch-utility LLM stage. Injected at the
   * extractor's construction site from the plugin-level batchChunkSize knob
   * (never parsed from the admissionControl config section); absent or
   * invalid values keep the BATCH_UTILITY_MAX_SIZE default. Clamped to the
   * same 1-50 range the extractor's own batched stages use.
   */
  ...
  model?: string;
  /**
   * "global" (default, also when absent): every admission call uses the
   * plugin's global llm model. "lane": the reflection-mapped admission gate
   * resolves the memoryReflection model instead, so the judge is never
   * dumber than the author whose rows it audits; extraction and fallback
   * admission stay on the global model.
   */
  modelAffinity?: "global" | "lane";
}
...
/** Which admission call site is resolving a model. */
...
export class AdmissionController {
  ...
  private batchChunkBound(): number {
  ...
  private async evaluateChunk(
    ...
    const preRejected = new Map<number, AdmissionEvaluation>();
    const scored: typeof chunk = [];
    ...
    const utilities =
      scored.length > 0 ? await this.scoreUtilityBatch(scored.map((item) => item.candidate)) : [];
